...
def collect_agreement_scores(model, inputs, targets, attention_mask_positions, tokenizer):
    ...
    if attention_mask_positions is not None:
        ...
        logger.info(f"Attention masking position {attention_mask_positions} in sequences")
    ...
    for dataset_key in selected_keys:
        ...
        if args.model_type == "unablated":
            ...
            model = ablate_attn_module(model, 
                                       layer_head_dict=literal_eval(args.lh_dict),
                                       ablation_type="zero")
        ...
        logger.info(f"Running experiment for:\n" +
