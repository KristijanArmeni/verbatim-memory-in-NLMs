"""
sample_noun_lists.py contains utility functions to generate the dataset

Example usage:
ipython %run sample_noun_lists.py --which "toronto" \
                                  --output_dir "./"
"""
...
def sample_tokens(token_list: list, step_size: int, window_size: int) -> list:
    ...
    if window_size > step_size:
        warnings.warn(
            "Window size larger than step size. Output samples "
...
def load_and_sample_noun_pool(
    path, which, model_vocab, n_items=None, n_lists=20, seed=None
):
    """
    ...
    if which == "random":
        ...
        df = pd.read_csv(full_path, sep="\t", header=0).rename(
            columns={"k-f freq": "k_f_freq"}
        )
        ...
        token_lists = sample_tokens(
            token_list=noun_pool[shuffle_ids].tolist()[0 : (n_items * n_lists)],
            step_size=n_items,
            window_size=n_items,
        )
        ...
        df["token_id"] = np.tile(
            np.arange(0, n_toks_per_set), n_sets
        )  # construct indices for each token
        df["set_id"] = np.repeat(
            np.arange(0, n_sets), n_toks_per_set
        )  # construct set indices for each set
        df["in_vocab"] = df["token"].isin(
            model_vocab
        )  # determine whether a token is in vocabulary
        ...
        df["valid_set"] = (
            df.n_valid > threshold
        )  # make sure there are at least 17 in vocabulary tokens
        ...
        print(
            "There are {} semantic token sets with at least {} in-vocabulary tokens".format(
                len(valid_sets), threshold
            )
        )
...
def get_noun_lists_toronto(which: str, list_length: int = 10):
    ...
    lists_of_tokens = load_and_sample_noun_pool(
        path=path,
        n_items=list_length,
        n_lists=23,
        which=which,
    ...
    lists_of_tokens_shifted = [
        np.roll(np.asarray(alist), s).tolist()
        for alist in lists_of_tokens
        for s in cshifts
    ]
    ...
    out_dict = {
        "n{}".format(n_items): [alist[0:n_items] for alist in lists_of_tokens_shifted]
    ...
    if which in ["ngram-random", "ngram-categorized"] or which == "ngram-distractors":
        ...
        tmp = {
            "n{}".format(n_gram): [
                list(
                    chunk(
                        np.tile(alist[0:n_gram], reps=n_reps).tolist(), n_gram, n_gram
                    )
                )
    ...
    # for ngram lists sample the created lists repeatedly
        ...
        out_dict = {
            "n{}".format(size): [
                list(chunk(thelist[0 : (n_reps_distractors * 7)], size, 7))
...
def chunk_lists_and_circular_shift(noun_list: list, list_length: int = 10) -> dict:
    ...
    lists_of_tokens_shifted = [
        np.roll(np.asarray(alist), s).tolist()
        for alist in lists_of_nouns
        for s in cshifts
    ]

    # make sure all are the same length (we drop residual, shorter lists)
    lists_of_tokens_shifted = [
        e for e in lists_of_tokens_shifted if len(e) == list_length
    ]
