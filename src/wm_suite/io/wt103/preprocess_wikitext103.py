...
def runtime_code():
    ...
    parser.add_argument(
        "--datadir", type=str, help="path to files with wikitext103 tokens"
    )
    parser.add_argument("--train_ds", type=str, help="path to training dataset")
    parser.add_argument("--val_ds", type=str, help="path to validation set")
    parser.add_argument("--test_ds", type=str, help="path to tokenized file")
    parser.add_argument(
        "--train_set_size", type=float, help="size of the training set (in M tokens)"
    )
    parser.add_argument(
        "--sequence_len",
        type=int,
    ...
    eos = "|<endoftext>|"
    train_toks_chunked = list(
        chunk(train_toks, args.sequence_len, equal_len=True, eos=eos, bos=eos)
    )
    val_toks_chunked = list(
        chunk(val_toks, args.sequence_len, equal_len=True, eos=eos, bos=eos)
    )
    test_toks_chunked = list(
        chunk(test_toks, args.sequence_len, equal_len=True, eos=eos, bos=eos)
    )
    ...
    train_input_ids = [
        tokenizer.convert_tokens_to_ids(chunk) for chunk in train_toks_chunked
    ]
    val_input_ids = [
        tokenizer.convert_tokens_to_ids(chunk) for chunk in val_toks_chunked
    ]
    test_input_ids = [
        tokenizer.convert_tokens_to_ids(chunk) for chunk in test_toks_chunked
    ]
