...
def gpt2_wt103(model, tokenizer, context_len, stride, batch_size):
    ...
    exp = Experiment(model=model, ismlm=False,
                    tokenizer=tokenizer,
    ...
    ppl, _, _ = exp.ppl(input_ids=torch.tensor([ids]), context_len=context_len, stride=stride)
...
def main(input_args=None):
    ...
    parser = argparse.ArgumentParser(description='Compute Wikitext-103 perplexity for different ablations of the GPT-2 model.')
    ...
    parser.add_argument("--model_type", type=str, choices=["unablated", "ablated", "rand-init"], default="unablated", help="model type")
    parser.add_argument('--lh_dict', type=str, default=None, help='layer-head dictionary')
    ...
    if args.model_type == "ablated":
        ...
        assert args.lh_dict is not None, "Please provide a layer-head dictionary (--lh_dict)"
    ...
    ppl = gpt2_wt103(model=model,
                     tokenizer=tokenizer,
                     context_len=args.context_len, 
                     stride=args.stride, 
                     batch_size=args.batch_size)

    out = pd.DataFrame({"ppl": ppl.cpu().item(),
                        "lh_dict": label,
                        "step": len(from_dict_to_labels(lh_dict))}, index=[0])
