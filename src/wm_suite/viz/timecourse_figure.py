...
def generate_plot(model: str):
    ...
    if model == "gpt2":
        ...
        data = load_csv_data(
            model=model, datadir=data_dir, fname="output_awd-lstm-3_a-10_sce1.csv"
        )
        data["model"] = "awd_lstm"
        ...
        data = load_csv_data(
            model=model, datadir=data_dir, fname=f"*{model}*sce1_1_*.csv"
        ).dropna()
        data["model"] = "gpt2"
    ...
    d.rename(
        columns={"list": "list structure", "second_list": "condition"}, inplace=True
    )
