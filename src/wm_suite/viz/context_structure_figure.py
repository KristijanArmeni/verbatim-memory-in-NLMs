...
titles = {
    "gpt2": "Transformer (Radford et al, 2019)",
    "w-12v2": "Transformer (WT103, 12-layer)",
    "awd_lstm": "LSTM (Merity et al, 2017)",
}
...
def generate_plot(model):
    ...
    for sce in ["sce1", "sce2", "sce1rnd"]:
        if model == "awd_lstm":
            ...
            dat_ = load_csv_data(
                model=model, datadir=data_dir, fname=f"output_awd-lstm-3_a-10_{sce}.csv"
            )
            dat_["model"] = model
            ...
            dat_ = load_csv_data(
                model=model, datadir=data_dir, fname=f"output_gpt2_a-10_{sce}.csv"
            )
            dat_["model"] = model
            tmp.append(dat_)
            ...
            dat_ = load_csv_data(model=model, datadir=data_dir, fname=fn3).dropna()
            dat_["model"] = "gpt2"
            dat_.prompt_len = dat_.prompt_len.map({5: 400})
            dat_.context = dat_.context.map(
...
def savefig(grid, savedir, figfname):
    ...
    print(f"Saving {fn}")
    grid.savefig(
        os.path.join(savedir, fn + ".pdf"), transparent=True, bbox_inches="tight"
    )
