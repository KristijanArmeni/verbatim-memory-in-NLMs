...
def plot_schematic(ax, d, colors, annot_fs, text_fs, squeezed=False):
    ...
    labels2 = {
        0: {
            "text": "Mary wrote down a list of words",
            "xpos": 0 * xmax,
            "ypos": ypos,
            "c": "tab:gray",
            "fc": "None",
        ...
        7: {
            "text": "...when she got back she read",
            "xpos": 0.56 * xmax,
            "ypos": ypos,
            "c": "tab:grey",
            "fc": "None",
...
def plot_imshow_and_average(
    ...
    if selection:
        lays = [
            ...
            if selection[i]
            for h in selection[i]
        ]
        for xy in lays:
            rect = patches.Rectangle(
                xy, 0.99, 0.99, linewidth=1.3, edgecolor="black", facecolor="none"
            )
        ...
        xycoord = [
            (h + 0.1, i) for i in selection.keys() if selection[i] for h in selection[i]
        ]
        xyinds = [
            (h, i) for i in selection.keys() if selection[i] for h in selection[i]
        ]
...
# %%
def attn_weights_per_head_layer(
    ax,
    x: np.ndarray,
    colors: List,
    img_text_colors: List,
    img_text_fs: int,
    query_id: int,
    target_id: int,
    ...
    selection, _, _ = find_topk_attn(
        x, topk=20, attn_threshold=0.2, tokens_of_interest=[target_id], seed=12345
    )
    _, img1, d1 = plot_imshow_and_average(
        ax[:, 0],
        xtmp,
        selection=selection,
        c=colors[2],
        img_txt_c=img_text_colors[0],
        img_txt_fs=img_text_fs,
    )
    ...
    selection, _, _ = find_topk_attn(
        x, topk=20, attn_threshold=0.2, tokens_of_interest=first_list, seed=12345
    )  # to mark top-10 cells
    _, img2, d2 = plot_imshow_and_average(
        ax[:, 1],
        xtmp,
        selection=selection,
        c=colors[0],
        img_txt_c=img_text_colors[1],
        img_txt_fs=img_text_fs,
    )
    ...
    selection, _, _ = find_topk_attn(
        x, topk=20, attn_threshold=0.2, tokens_of_interest=preceding, seed=12345
    )  # to mark top-10 cells
    _, img3, d3 = plot_imshow_and_average(
        ax[:, 2],
        xtmp,
        selection=selection,
        c=colors[1],
        img_txt_c=img_text_colors[2],
        img_txt_fs=img_text_fs,
    )
...
def make_plot(datadir: str, query: str) -> Tuple[plt.Figure, pd.DataFrame]:
    ...
    if query == "colon-colon-p1":
        ...
        query_idx = 45
        suptitle = (
            "Copy and previous-token attention heads in GPT-2 (single-token window)"
        )
    ...
    fontcfg = get_font_config(
        pymodule=os.path.basename(__file__),
        currentfont=plt.rcParams["font.sans-serif"][0],
    )
    ...
    if query in [
        ...
        if query in ["colon-colon-p1"]:
            plot_schematic(
                ax1,
                d1,
                text_fs=fontcfg["schema_text_fs"],
                annot_fs=fontcfg["schema_annot_fs"],
                colors=clrs,
                squeezed=False,
            )
        else:
    ...
    # ===== PLOTTING FUNCTION ===== #
    ax, imgs, data1 = attn_weights_per_head_layer(
        ax=axes,
        x=x1,
        target_id=target_id,
        query_id=query_idx,
    ...
    for a in ax[0, :]:
        ...
        a.set_yticklabels(
            [str(i) if i % 2 != 0 else "" for i in range(1, 13, 1)],
    ...
    cbar.ax.set_ylabel(
        "Attention weight", fontsize=fontcfg["labelfs"], fontweight="semibold"
    )
...
def main(input_args=None):
    ...
    if args.which is None:
        raise ValueError(
            "Please specify which figures to make by providing '--which' argument"
        )
    ...
    if main_fig:
        ...
        if args.savedir:
            save_png_pdf(
                fig1, os.path.join(args.savedir, "gpt2_attn_colon-colon-p1_v2")
            )
    ...
    # single token plot
    if single_token_fig:
        fig4, fig4_, data = make_plot(
            datadir=datadir, query="colon-colon-p1-single-token"
        )
        if args.savedir:
            save_png_pdf(
                fig4,
                os.path.join(args.savedir, "gpt2_attn_colon-colon-p1-single-token"),
            )
    ...
    if first_noun_fig:
        ...
        if args.savedir:
            ...
            save_png_pdf(
                fig5_, os.path.join(args.savedir, "gpt2_attn_colon-colon-n1_control")
            )
