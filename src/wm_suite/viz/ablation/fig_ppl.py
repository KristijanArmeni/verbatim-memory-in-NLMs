...
def make_plot(datadir: str):
    ...
    ax.plot(dat["step"], np.log(dat["ppl"]), "--o", label="Heads found in\nattention-informed search")
    ax.plot(dat2["step"], np.log(dat2["ppl"]), "--^", label="Heads found in\nUnconstrained search")

    ax.hlines(np.log(dat_unablated["ppl"]), 1, len(dat["step"]), linestyle="--", color="black", label="Unablated model")
    ax.hlines(np.log(dat_all["ppl"]), 1, len(dat["step"]), linestyle="-.", color="tab:red", label="Fully ablated model\n(all heads)")

    annot_str = f"Randomly initialized model ppl: {np.log(dat_random['ppl'].item()):.1f}"
    ax.text(3, 4.1, s=annot_str, fontsize=fontcfg["annotfs"])

    ax.set_title("Does ablating heads important for verbatim retrieval affect next-word prediction?\n(performance on Wikitext103 test set)", fontsize=fontcfg["titlefs"])
    ...
    ax.set_xticklabels(["" if i%2 == 0 else i for i in np.arange(1, len(dat["step"])+1, 1)], fontsize=fontcfg["tickfs"])
