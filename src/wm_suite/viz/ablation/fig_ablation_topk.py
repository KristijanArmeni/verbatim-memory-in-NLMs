...
from src.wm_suite.wm_ablation import (
    find_topk_attn,
    from_dict_to_labels,
    from_labels_to_dict,
)
...
def construct_filnames():
    ...
    control_labels = (
        [f"copying-ctrl{i}" for i in range(5)]
        + [f"previous-ctrl{i}" for i in range(5)]
        + [f"matching-ctrl{i}" for i in range(5)]
        + [f"copying-and-matching-ctrl{i}" for i in range(5)]
    )
    ...
    for tup in combinations:
        k, head_type = tup
        csvs[
            f"ablate-top{k}-{head_type}"
        ] = f"ablation_gpt2_ablate-top{k}-{head_type}_sce1_1_3_random_repeat.csv"
        jsons[f"ablate-top{k}-{head_type}"] = f"gpt2_ablate-top{k}-{head_type}_ppl.json"

    csvs[
        "matching-bottom5"
    ] = "ablation_gpt2_ablate-matching-bottom5_sce1_1_3_random_repeat.csv"
    jsons["matching-bottom5"] = "gpt2_ablate-matching-bottom5_ppl.json"

    csvs[
        "induction-matching-intersect"
    ] = "ablation_gpt2_ablate-induction-matching-intersect_sce1_1_3_random_repeat.csv"
    jsons[
        "induction-matching-intersect"
    ] = "gpt2_ablate-induction-matching-intersect_ppl.json"
...
# %%
def load_memory_ppl_files(
    datadir: str, csvfiles: Dict, jsonfiles: Dict
) -> List[pd.DataFrame]:
    dfs = []
...
def select_conditions(
    labels: np.ndarray, substring: str, match: str = "substring"
) -> np.ndarray:
    if match == "substring":
...
# %%
def aggregate_over(
    array: np.ndarray, labels: np.ndarray, substring: str, match: str, agg_func
):
    """
...
# %%
def prepare_data(
    d_mem, d_ppl, d_sva, labels, target_label, labels_sva, target_label_sva
):
    ids = []
    ids_sva = []
    for k in [5, 10, 15, 20]:
        ids.append(
            select_conditions(
                labels, f"ablate-top{k}-{target_label}", match="exact"
            ).tolist()[0]
        )
        ids_sva.append(
            select_conditions(
                labels_sva, f"top{k}-{target_label_sva}", match="exact"
            ).tolist()[0]
        )
    ...
    for i, k in enumerate([5, 10, 15, 20]):
        agg_func = np.mean
        yA_ctrl[i, :] = aggregate_over(
            d_mem,
            labels,
            f"top{k}-{target_label}-ctrl",
            match="substring",
            agg_func=agg_func,
        )
        yB_ctrl[i, :] = d_ppl[
            select_conditions(labels, f"top{k}-{target_label}-ctrl", match="substring"),
            ...,
        ]
        yC_ctrl[i, :] = d_sva[
            select_conditions(
                labels_sva, f"top{k}-{target_label_sva}-ctrl", match="substring"
            ),
            ...,
        ]
...
def plot_data_points(ax, y):
    ...
    for i in range(y.shape[-1]):
        ...
        ax.scatter(
            ((np.arange(len(y)) + 1) + xoffset) + yjitter,
            y[:, i],
            s=40,
            color="tab:gray",
            ec="white",
...
def get_ci(y):
    ...
    ci = np.zeros((2, 1))
    bs = bootstrap(
        (y,), axis=0, statistic=np.mean, confidence_level=0.95, n_resamples=10000
    )
...
def make_memory_plot(axes0, data1, data2, data3, data4, fs):
    ...
    for i, k in enumerate([1, 2, 3, 4]):
        ...
        plot_box_data(
            axes0[2], k - offset, y3_mem[i, :], horizontal=False, c=clrs_deep[1], a=0.3
        )
        plot_box_data(
            axes0[2],
            k + offset,
    ...
    for a in axes0:
        a.hlines(
            ...
            linestyle="--",
            color="black",
            label="Score indicating\nno memory",
        )
        a.hlines(
            y=np.median(unablated),
            xmin=a.get_xlim()[0],
            xmax=a.get_xlim()[1],
            linestyle="-.",
            color="black",
        ...
        a.fill_between(
            x=np.arange(a.get_xlim()[0], a.get_xlim()[1]),
            y1=ci[0],
            y2=ci[1],
            alpha=0.2,
            color="black",
...
# %% make_ppl_plot
def make_ppl_plot(
    axes, data1: Tuple, data2: Tuple, data3: Tuple, data4: np.ndarray, fs: int
):
    y1_ppl, y1_ppl_ctrl = data1  # matching heads
    y2_ppl, y2_ppl_ctrl = data2  # postmatching heads
    y3_ppl, y3_ppl_ctrl = data3  # recent-tokens heads
    ...
    ax = plot_data_points(axes[0], y1_ppl_ctrl - ppl_unabl)
    ...
    ax = plot_data_points(axes[1], y2_ppl_ctrl - ppl_unabl)
    ...
    ax = plot_data_points(axes[2], y3_ppl_ctrl - ppl_unabl)

    # for a in axes:
    #    a.hlines(y=ppl_unabl, xmin=a.get_xlim()[0], xmax=a.get_xlim()[1], linestyle="-.", color="black", label="Unablated model")

    leg = axes[1].legend(fontsize=fs - 3)
    leg.legendHandles[0].set_fc("none")
    leg.legendHandles[0].set_ec("darkgray")
    leg.legendHandles[0].set_ec("darkgray")

    axes[0].set_ylabel("Test set loss\n(change from unablated)", fontsize=fs)
    axes[0].set_yticks([0, 1, 2, 3])
    axes[0].set_yticklabels([0, 1, 2, 3], fontsize=fs)
    ...
    axes[0].set_title(
        "Matching heads", fontweight="semibold", color=clrs.green, fontsize=fs
    )
    axes[1].set_title(
        "Post-match heads", fontweight="semibold", color=clrs.blue, fontsize=fs
    )
    axes[2].set_title(
        "Recent-tokens heads", fontweight="semibold", color=clrs.orange, fontsize=fs
    )
...
def make_sva_plot(axes, data1, data2, data3, data4, fs):
    ...
    ax = plot_bar(
        axes[0], y=y1_sva, xoffset=0.2, color=clrs.green, barwidth=0.4, label=None
    )
    ax = plot_bar(
        axes[0],
        y=y1_sva_ctrl_m,
    ...
    ax = plot_bar(
        axes[1], y=y2_sva, xoffset=0.2, barwidth=0.4, color=clrs.blue, label=None
    )
    ax = plot_bar(
        axes[1],
        y=y2_sva_ctrl_m,
    ...
    ax = plot_bar(
        axes[2], y=y3_sva, xoffset=0.2, barwidth=0.4, color=clrs.orange, label=None
    )
    ax = plot_bar(
        axes[2],
        y=y3_sva_ctrl_m,
...
def generate_plot(datadir, which):
    ...
    y4_mem, y4_mem_ctrl, y4_ppl, y4_ppl_ctrl, y4_sva, y4_sva_ctrl = prepare_data(
        y_mem,
        y_ppl,
        y_sva,
        xlabels,
    ...
    y5_ids = select_conditions(
        xlabels, f"induction-matching-intersect", match="exact"
    ).tolist()[0]
    y5_sva_ids = select_conditions(
        sva_xlabels, f"induction-matching-intersect", match="exact"
    ).tolist()[0]
    y5_mem = y_mem[y5_ids, :]
    y5_ppl = (y_ppl[y5_ids],)
    ...
    y6_sva_ids = select_conditions(
        sva_xlabels, f"matching-bottom5", match="exact"
    ).tolist()[0]
    y6_mem = y_mem[y6_ids, :]
    y6_ppl = (y_ppl[y6_ids],)
    ...
    # top row
    if which == "memory_fig":
        y1_mem, y1_mem_ctrl, _, _, _, _ = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "matching", sva_xlabels, "matching"
        )
        y2_mem, y2_mem_ctrl, _, _, _, _ = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "copying", sva_xlabels, "copying"
        )
        y3_mem, y3_mem_ctrl, _, _, _, _ = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "previous", sva_xlabels, "previous"
        )

        unabl_ids = select_conditions(xlabels, "unablated", match="exact").tolist()[0]
        unabl_mem = y_mem[unabl_ids]

        fig, axes0 = plt.subplots(1, 3, figsize=(8, 3.5), sharey="all")
        ...
        _, _, y1_ppl, y1_ppl_ctrl, _, _ = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "matching", sva_xlabels, "matching"
        )
        _, _, y2_ppl, y2_ppl_ctrl, _, _ = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "copying", sva_xlabels, "copying"
        )
        _, _, y3_ppl, y3_ppl_ctrl, _, _ = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "previous", sva_xlabels, "previous"
        )

        unabl_ids = select_conditions(xlabels, "unablated", match="exact").tolist()[0]
        unabl_ppl = y_ppl[unabl_ids]
        ...
        _, _, _, _, y1_sva, y1_sva_ctrl = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "matching", sva_xlabels, "matching"
        )
        _, _, _, _, y2_sva, y2_sva_ctrl = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "copying", sva_xlabels, "copying"
        )
        _, _, _, _, y3_sva, y3_sva_ctrl = prepare_data(
            y_mem, y_ppl, y_sva, xlabels, "previous", sva_xlabels, "previous"
        )
...
def main(input_args=None):
    ...
    input_args = [
        "--datadir",
        "C:\\users\\karmeni1\\project\\lm-mem\\data\\ablation\\zero_attn\\topk",
        "--which",
        "ppl_fig",
        "--savedir",
        "C:\\users\\karmeni1\\project\\lm-mem\\fig\\ablation\\topk",
    ]
    ...
    if args.which == "memory_fig":
        ...
        fig2.savefig(
            os.path.join(savedir, "matching_and_post-matching_memory.png"), dpi=300
        )
        ...
        attn_dict = dict(
            np.load(
                "C:\\users\\karmeni1\\project\\lm-mem\\data\\ablation\\attention_weights_gpt2_colon-colon-p1.npz"
            )
        )
        toi = [13]
        topk = [5, 10, 15, 20]
        ...
        def plot_head_selection1(fig, ax, layer_head_dict, vals, cmap):
            ...
            for xy in lays:
                rect1 = patches.Rectangle(
                    xy, 0.35, 0.35, linewidth=1.5, facecolor="tab:red"
                )
                ax.add_patch(rect1)
        ...
        def plot_head_selection(seed):
            ...
            for j, k in enumerate(topk):
                sel_dict, ctrl_dict, vals = find_topk_attn(
                    attn_dict["data"], k, tokens_of_interest=toi, seed=seed
                )
                ...
                lays = [
                    (i + 0.08, h - 0.15)
                    for i in ctrl_dict.keys()
                    if ctrl_dict[i]
                    for h in ctrl_dict[i]
                ]
                for xy in lays:
            ...
            fig.supxlabel("Layer")
            plt.suptitle(
                f"{toi_name.capitalize()}-token and control heads (seed {seed})",
                fontsize=16,
            )
        ...
        lh_dict, _, vals = find_topk_attn(
            attn_dict["data"], 20, tokens_of_interest=[13, 14], seed=12345
        )
        ax = plot_head_selection1(fig5, ax, lh_dict, vals, cmap="Greys")
        ax.set_title(
            "Top-20 copying and matching heads\n(attending to matching token at t and t+1)"
        )
        plt.tight_layout()
        plt.show()
        fig5.savefig(
            os.path.join(savedir, "top20_copying-and-matching_heads.png"), dpi=300
        )
        ...
        top20_matching, _, vals = find_topk_attn(
        ...
        for key in abl_dicts.keys():
            sel_dict, _, vals = find_topk_attn(
                attn_dict["data"], k, tokens_of_interest=abl_dicts[key], seed=12345
            )
        ...
        for i, x in enumerate(d):
            ...
            ax.bar(
                np.arange(len(counts)) + offsets[i],
                height=counts,
                width=0.25,
                label=labels[i],
            )
        ...
        ax.set_title(
            f"Distribution of head types (top-{k}) across GPT-2 layers", fontsize=16
        )
