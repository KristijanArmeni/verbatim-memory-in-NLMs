...
def pairs2list(attn, which):
    ...
    pairs_list = [
        "-".join([s.replace(".", "") for s in from_dict_to_labels(e)])
        for e in get_pairs(lh_dict)
    ]
...
def save_pair_labels_to_json(savedir):
    attn_data = os.path.join(
        PATHS.data, "ablation\\attention_weights_gpt2_colon-colon-p1.npz"
    )
...
def get_repeat_surprisals(dataframes):
    ...
    for l in range(len(dataframes)):
        ...
        dats_[dataframes[l].model_id.unique().item()] = np.median(
            dat_.x_perc.to_numpy()
        )
...
def populate_data_matrix(data_dict):
    ...
    unq = get_unique_labels(
        data_dict.keys()
    )  # find unique labels (i.e. labels of top20 heads)
...
def plot_imshow(ax, data, labels, vmin=0, vmax=100, cmap=plt.cm.Greys):
    ...
    minrow, mincol = np.where(
        np.tril(data) == np.min(data.flatten()[data.flatten().nonzero()])
    )
    maxrow, maxcol = np.where(
        np.tril(data) == np.max(data.flatten()[data.flatten().nonzero()])
    )
    ...
    for xy in [(mincol.item(), minrow.item()), (maxcol.item(), maxrow.item())]:
        ...
        rect1 = patches.Rectangle(
            (x, y), 1, 1, linewidth=1.3, edgecolor="red", facecolor="none"
        )
        ax.add_patch(rect1)
...
def main(input_args=None):
    ...
    get_min_max = (
        lambda x: f"Min = {np.round(np.min(x), 1)}%, Max = {np.round(np.max(x),1)}%"
    )
    ...
    ax1, im1 = plot_imshow(
        ax[0],
        data1,
        [reformat_label(e) for e in matching_labels],
        vmin=vmin,
        vmax=vmax,
        cmap=plt.cm.viridis,
    )
    ...
    ax1.set_title(
        "Matching heads", color="#2ca02c", fontweight="bold", fontsize=titlefs
    )
    ...
    ax2, im2 = plot_imshow(
        ax[1],
        data2,
        [reformat_label(e) for e in postmatching_labels],
        vmin=vmin,
        vmax=vmax,
        cmap=plt.cm.viridis,
    )
    ...
    ax2.set_title(
        "Post-matching heads", color="#1f77b4", fontweight="bold", fontsize=titlefs
    )
    ...
    ax3, im3 = plot_imshow(
        ax[2],
        data3,
        [reformat_label(e) for e in recent_labels],
        vmin=vmin,
        vmax=vmax,
        cmap=plt.cm.viridis,
    )
    ...
    ax3.set_title(
        "Recent-tokens heads", color="#ff7f0e", fontweight="bold", fontsize=titlefs
    )
