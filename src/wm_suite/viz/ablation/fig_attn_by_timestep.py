...
def load_data(datadir, token_string: str) -> Tuple[np.ndarray, Dict]:
    fn = os.path.join(
        datadir, f"attention_weights_gpt2_colon-colon-{token_string}-ctxlen1_llen10.npz"
    )
...
p1_tokens = load_data(datadir, "p1")[-1][
    "tokens"
...
lh_dict_match, _, match_vals = find_topk_attn(
    x, topk=20, attn_threshold=0.15, tokens_of_interest=[13], seed=12345
)
lh_dict_post, _, post_vals = find_topk_attn(
    x, topk=10, attn_threshold=0.2, tokens_of_interest=[14], seed=12345
)
lh_dict_recent, _, recent_vals = find_topk_attn(
    x, topk=10, attn_threshold=0.2, tokens_of_interest=[58], seed=12345
)
# %%
...
median_line = ax[0].plot(
    np.median(datnan[0 : marker1_ons + 1, :], axis=1), "-", color=lc
)[0]
...
marker1 = ax[1].vlines(
    marker1_ons, len(ylabs) - 0.5, len(ylabs) - 1, color="tab:red", linewidth=2
)
...
if marker2_ons is None:
    ...
    marker2 = ax[1].vlines(
        marker2_ons, len(ylabs) - 0.5, len(ylabs) - 1, color="tab:red", linewidth=2
    )
...
ax[1, 0].plot(
    np.arange(len(dm2["labs"]))[1::], dm2["attn"][1::], "--o", color=clrs.green
)
ax[1, 1].plot(
    np.arange(len(dp2["labs"]))[1::], dp2["attn"][1::], "--^", color=clrs.blue
)
ax[1, 2].plot(
    np.arange(len(dr2["labs"]))[1::], dr2["attn"][1::], "--s", color=clrs.orange
)
...
ax[1, 1].set_title(
    "Top-10 post-match heads\n(target token occurs at multiple positions)"
)
ax[1, 2].set_title(
    "Top-10 recent-tokens heads\n(target token occurs at multiple positions)"
)
...
fig.supxlabel(
    "Target token (sequence position index)\nSource token (sequence position index)"
)
