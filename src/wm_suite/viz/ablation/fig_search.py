...
data1_ = load_json(os.path.join(PATHS.search, files["topk_n1_list"]))        # based on a single token
data2_ = load_json(os.path.join(PATHS.search, files["topk_n2_list"]))
data4_ = load_json(os.path.join(PATHS.search, files["topk_n1_list_orig"]))   # aggregated across 3-tokens etc.
...
def random_search_across_all_heads():
    ...
    fig6, axes6 = plot_search_trajectory(data6, memb_dict, {0: "tab:green", 1: "tab:blue", 2: "tab:orange", 3: "tab:grey"})
    axes6[0].set_title("Greedy search results across all heads (effects on N2+N3)")
    axes6[1].set_title("Searched heads")

    axes6[0].hlines(rand_n1_n2["rs"]["median"], 0, 20, linestyle="--", color="tab:purple", label="Rand. init.")
    axes6[0].hlines(all_n1_n2["rs"]["median"], 0, 20, linestyle="-.", color="tab:red", label="All heads ablated")
...
save_png_pdf(fig6, savename=os.path.join(PATHS.root, "fig", "ablation", "topk", "fig_search_n2_n3_all-heads"))
...
def plot_attention_based_n2_n3_from_colon(data, membership):
    ...
    axes[0].hlines(rand_n1["rs"]["median"], 0, axes[0].get_xlim()[-1], linestyle="--", color="tab:purple", label="Rand. init.")
    axes[0].hlines(all_n1["rs"]["median"], 0, axes[0].get_xlim()[-1], linestyle="-.", color="tab:red", label="All heads ablated")

    ci = rand_n1["rs"]["ci95"]
    axes[0].fill_between(np.arange(43), y1=ci[0], y2=ci[1], color="tab:purple", alpha=0.3)
    ci = all_n1["rs"]["ci95"]
    axes[0].fill_between(np.arange(axes[0].get_xlim()[-1]), y1=ci[0], y2=ci[1], color="tab:red", alpha=0.3)
    axes[0].legend()
...
save_png_pdf(fig, savename=os.path.join(PATHS.root, "fig", "ablation", "topk", "fig_search_n2_n3_random"))
...
for i, h in enumerate(sel_heads):
    ...
    ax[i] = plot_heads(ax[i], h, {0: "tab:green", 1: "tab:blue", 2: "tab:orange", 3: "tab:grey"}, memb_dict)
    ax[i].set_title(f"Search {i+1} ({max_vals[max_searches[i]]:.1f}%, N={len(h)})", fontweight="semibold")
...
save_png_pdf(fig, savename=os.path.join(PATHS.root, "fig", "ablation", "topk", "fig_search_n2_n3_random_overlap"))
...
n_possible_searches = 43
rnd_mat_neg = np.full(shape=(len(rnd), n_possible_searches), fill_value=np.nan, dtype=float)
for i, r in enumerate(rnd):
...
axes[0].hlines(rand_n1_n2["rs"]["median"], 0, 35, linestyle="--", color="tab:purple", label="Rand. init.")
axes[0].hlines(all_n1_n2["rs"]["median"], 0, 35, linestyle="-.", color="tab:red", label="Ablate all")
...
axes[0].set_yticklabels([i if i%20 == 0 else "" for i in np.arange(0, np.nanmax(rnd_mat_neg)+10, 10)])
axes[0].set_title("Greedy search results across all head types\n(effects on N2+N3)")
axes[1].set_title("Searched heads")
plt.tight_layout()
plt.show()
...
save_png_pdf(fig, savename=os.path.join(PATHS.root, "fig", "ablation", "topk", "fig_search_n2_n3_neg_random"))
