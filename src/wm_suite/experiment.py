...
class Dataset(object):
    ...
    def tokens_to_ids(self, check_unkns=True):
        ...
        for seq in self.seqs:
            ...
            self.seq_ids.append(
                torch.tensor(ids, dtype=torch.int64).unsqueeze(-1).unsqueeze(-1)
            )
...
class Experiment(object):
    ...
    def run(self, input_set, metainfo):
        ...
        for j, sequence in enumerate(tqdm(input_set, desc="sequence")):
            ...
            if self.rnn_type in ["QRNN", "AWD_LSTM"]:
                ...
                model_output = self.evaluate_awd(
            ...
            model_output.states = [
                s.to_numpy_arrays() if store_states else s for s in states
            ]

            model_output.meta = {key: metainfo[key][j] for key in metainfo.keys()}

            # conver .step and .log_probs to arrays
            output.append(model_output.to_numpy_arrays().drop_eos_tokens())
    ...
    def evaluate_awd(self, data_source, hidden_size, bptt, batch_size=10):
        ...
        for i in range(0, data_source.size(0) - 1, bptt):
            data, targets = get_batch_awd(
                data_source, i, bptt, seq_len=None, evaluation=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint_folder", type=str, help="path/to/folder in which model is save"
    )
    parser.add_argument(
        "--model_weights", type=str, help="path/to/model_weights.pt of LSTM model"
    )
    parser.add_argument(
        "--vocab_file",
        type=str,
        default="./rnn_/vocab.text",
    ...
    parser.add_argument(
        "--output_filename", type=str, help="output_file.csv - file with results"
    )
    ...
    if args.vocab_file == "awd_lstm_corpus":
        data = torch.load(
            "/home/ka2773/project/lm-mem/src/src/wm_suite/awd_lstm/data/wikitext-103/corpus.844da84f12f45a4f741beb331daea986.data"
        )
    ...
    datadict = [
        {
            ...
            if "prompt_len" in o.meta.keys()
            else np.nan,
            "ppl": o.ppl,
            "log_probs": np.squeeze(o.log_probs)
            if o.log_probs.ndim > 1
            else o.log_probs,
    ...
    def datanamespace_to_df(data):
        ...
        for i, el in enumerate(tqdm(data, desc="sequence")):
            ...
            if args.per_token_output:
                ...
                input_arrays = [np.array(el.token), el.step, el.markers, el.log_probs]
                ...
                tmp[
                    ...
                    el.stimid
                )  # read-in from markers file, counts over conditions (0-229)
            ...
            tmp[
                "sentid"
            ] = el.trial  # this counts over all input sequences in the experiment
