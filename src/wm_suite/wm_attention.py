...
def get_attention_weights(
    ...
    attn, labs = [], []
    with torch.no_grad():
        for inputs, from_token in tqdm(
            zip(sequences, query_tokens_ids), desc="sequence:", total=len(sequences)
        ):
            o = model(
            ...
            attns = [
                layer[:, :, from_token, :] for layer in o.attentions
            ]  # layer.shape = (batch_size, n. heads, sequence_len, sequence_len)

            # concatenate layer dimension across dimension 0
            attn.append(
                torch.cat(attns, dim=0)
            )  # shape = (layers, heads, sequence_len)
...
def find_cue_token_ids(markers: np.ndarray) -> Tuple[int, int]:
    ...
    a = np.where(markers == prefix_code)[0][
        -1
    ]  # find the final token in the prefix (coded as 1) (":" etc.)
    b = np.where(markers == intervening_text_code)[0][
        -1
    ]  # find the final token in the intervening text (coded as 2)
...
def find_list_punctuation_index(markers, subtok_markers):
    ...
    a = np.where(
        (markers == second_list_marker) & (subtok_markers == punctuation_marker)
    )[0]

    return a


def replace_cue_symbols(
    sequences: List, tokens: List, markers: List, symbols: Tuple[str, str], tokenizer
) -> Tuple[List, List]:
    """
...
def get_args_for_devtesting():
    input_args = [
        ...
        "",
        "--savedir",
        "/scratch/ka2773/project/lm-ecog/data/attn",
        "--checkpoint",
        "gpt2",
        "--tokenizer",
        "gpt2",
...
def main(input_args=None, devtesting=False):
    ...
    if args.state_dict:  # for loading ablation models
        ...
        configuration = (
            model_unablated.config
        )  # find the config of the pretrained model
        model = GPT2LMHeadModel(
            configuration
        )  # use that config to initialized class for ablated model
    ...
    word_lists1, word_lists2 = make_word_lists(
        args.noun_list_file, condition=args.condition
    )
    ...
    input_sequences, input_sequences_info = concat_and_tokenize_inputs(
        prompt=prompts[args.scenario][args.prompt_len],
    ...
    input_sequences_info["prompt"] = [
        args.prompt_len for _ in input_sequences_info["list_len"]
    ]
    ...
    if args.query_token == "p1":
        query_idxs = [
            find_cue_token_ids(np.array(markers))[1]
            for markers in input_sequences_info["trialID"]
        ]

    elif args.query_token in ["r1", "r2", "r3", "r4"]:
        tag2id = {"r1": 1, "r2": 2, "r3": 3, "r4": 4}
        offset_from_comma = tag2id[
            args.query_token
        ]  # get number of positions to subtract from comma position
        query_idxs = [
            find_cue_token_ids(np.array(markers))[1] - offset_from_comma
            for markers in input_sequences_info["trialID"]
        ]
        ...
        if args.query_token == "r1":
            assert (
                sum([strings[i][idx] == "Ġagain" for i, idx in enumerate(query_idxs)])
                == 230
            )
        elif args.query_token == "r2":
            assert (
                sum([strings[i][idx] == "Ġlist" for i, idx in enumerate(query_idxs)])
                == 230
            )
        elif args.query_token == "r3":
            assert (
                sum([strings[i][idx] == "Ġthe" for i, idx in enumerate(query_idxs)])
                == 230
            )
        elif args.query_token == "r4":
            assert (
                sum([strings[i][idx] == "Ġread" for i, idx in enumerate(query_idxs)])
                == 230
            )

    # if the query token is a noun in the list, find it by looking for list marker and puncutation markers
    elif args.query_token in [
        "n1",
        "n2",
        "n3",
        "n4",
        ...
        query_idxs = [
            find_list_punctuation_index(np.array(markers), np.array(punct_markers))[
                punct_id
            ]
            - index_offset
            for markers, punct_markers in zip(
                input_sequences_info["trialID"], input_sequences_info["subtok"]
            )
        ]
    ...
    control_cue_tokens = [";", ","]
    if (args.list1_cue_symbol in control_cue_tokens) or (
        args.list2_cue_symbol in control_cue_tokens
    ):
        logging.info("Replacing one (or both) of the list cue symbols...")

        input_sequences, strings = replace_cue_symbols(
    ...
    r = get_attention_weights(
        model.to(device), input_sequences, query_tokens_ids=query_idxs, device=device
    )
    ...
    for d, s in zip(r, strings):
        bpe_splits = mark_subtoken_splits(
            tokens=s,
