...
def reproduce_set_size_experiment():
    ...
    runs = list(product(vignettes, conditions, list_lengths, list_types, intervening_texts))
    ...
    for run in runs:
        ...
        fn, fn2 = make_filename_from_config(model=args.model, 
                                            vignette=vignette,
                                            condition=condition, 
                                            list_len=list_len,
                                            list_type=list_type,
        ...
        input_arguments = [
            ...
            "--inputs_file_info", os.path.join(DATAPATH, fn2),
            "--tokenizer", args.model,
            "--checkpoint", args.model,
            "--device", f"{args.device}",
            "--batch_size", f"{args.bs}",
            "--output_dir", f"{os.path.abspath(args.output_dir)}",
            "--output_filename", f"wms_gpt2_a-10_{vignette}_{intervening_text}_n{list_len}_{condition}_{list_type}.csv"
        ]
