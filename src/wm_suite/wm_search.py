...
def greedy_search(
    ...
    while early_stop.check(
        search_space=attention_heads, patience=patience, counter=counter
    ):
        ...
        print("\n====================================")
        logger.info(
            f"Doing search (N_iterations = {len(attention_heads)} | patience = {patience}/{patience_limit} | max_iter = {max_iter} | Save = every {save_every} iterations)"
        )
        ...
        for i, new_head in enumerate(attention_heads):
            ...
            input_args = [
                ...
                "ablated",
                "--model_id",
                f"ablate_{i}_{new_head}",
                "--aggregate_output",
                "--aggregate_positions",
                f"{repeat_surprisal_timesteps}",
            ...
            if log_path:
                ...
                tmpfn = os.path.join(
                    log_path, f"log_scores_{log_id+'_'}{counter:02d}.json"
                )
                tmpdict = {
        ...
        increment = (
            best_score - best_scores[-1] if best_scores else 1
        )  # dummy value to keep the code running
        patience_delta = 0.5
    ...
    logger.info(
        f"Iterations ran: {len(best_scores)}/{n_total_runs} | patience: {patience}/{patience_limit}"
    )
    logger.info(
        f"Found best score: {np.max(best_scores)} for combination: {' '.join(best_labels[np.argmax(best_scores)])}"
    )
...
def get_input_args_for_devtesting():
    out = [
        "--lh_dict_json",
        "/home/ka2773/project/lm-mem/data/topk_heads/all_heads.json",
        "--log_id",
        "test",
        "--repeat_surprisal_timesteps",
        "[1,2]",
        "--list_len",
        "3",
        "--patience_limit",
...
def main(input_args=None, devtesting=False):
    ...
    # save the outputs
    logger.info(
        f"Saving outputs to {os.path.join(args.output_dir, args.output_filename)}"
    )
