...
LAKRETZ2021 = "/scratch/ka2773/project/lm-mem/sv_agr/lakretz2021/short_nested_inner_english.json"
...
strings = [[s.split()[0].split("'")[0].strip(',').strip('.') for s in list(e['target_scores'].keys())] for e in d1]
iscorr = [[i for i in list(e['target_scores'].values())] for e in d1]

# get the grammatical number of the first target (coded as 's' or 'p')
targets = [(str(p.compare_verbs(e[0], e[1])).split(":")[0], iscorr[i][0]) for i, e in enumerate(strings) if len(e) == 2]
